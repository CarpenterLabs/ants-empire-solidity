...
contract QuestPool is BasePool {
    ...
    constructor(address _nodeWallet, address _nectarAddress) BasePool(_nodeWallet, _nectarAddress) {}
    ...
    function sendFunds(address recipient, uint256 amount) external onlyAllowed {
        ...
        require(amount <= IERC20(nectarToken).balanceOf(address(this)), "Insufficient contract balance");

        // Transfer the tokens to the recipient
        IERC20(nectarToken).safeTransfer(recipient, amount);
        // require(IERC20(nectarToken).transfer(recipient, amount), "Token transfer failed");
